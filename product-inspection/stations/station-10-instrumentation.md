...
| Key-flow tracking | Are the core jobs (Station 7) instrumented as funnels? | Interview |
...
2. **Check analytics depth.** Is anything tracked beyond raw pageviews? Are *meaningful events* — the core actions from Station 7 — instrumented, so the team can answer "how many people actually complete the primary job?"
3. **Check funnel visibility.** For each core job, can the team see where users drop off? Without this, Station 7 findings can never be confirmed by real data.
4. **Check performance monitoring.** Is there real-user monitoring (web vitals from the field), or only occasional lab tests? Field data is what catches the regression Station 5 can't predict.
...
- Instrument the core jobs from Station 7 as events/funnels — that's what lets future inspections check claims against real behavior instead of interview.
